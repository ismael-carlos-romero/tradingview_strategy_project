        ...
        if sys_state in ["STOP", "DISARMED"]:
            ...
            if not is_online:
                msg_offline = (
                    ...
                    f"Se registra la señal en Sheets como <i>EXECUTION_MISSED_UCHARTS</i>."
                )
                try:
    ...
    while True:
        ...
        try:
            ...
            if desired == 'RUNNING':
                ...
                # Procesar señales de compra encoladas si el autotrader general esta activo
                state = get_system_state()
                if state == "ARMED":
                    global PENDING_TRADES_QUEUE
                    if PENDING_TRADES_QUEUE:
                        # Extraer la primera señal de la cola
                        trade_req = PENDING_TRADES_QUEUE.pop(0)
                        ticker = trade_req.get("ticker")
                        strategy = trade_req.get("strategy_id") or trade_req.get("strategy")
                        price = trade_req.get("price")
                        qty = trade_req.get("quantity", 1)
                        
                        print(f"[Worker paper_bot] Procesando compra automatica de la cola para {ticker} ({strategy})...")
                        db_conn = None
                        try:
                            # Ejecutar la compra en el ledger del simulador
                            db_conn = sqlite3.connect(DB_NAME, timeout=30.0)
                            res = sim_broker_core.simular_compra_broker(db_conn, ticker, strategy, price, cantidad_propuesta=qty)
                            
                            c = db_conn.cursor()
                            if res.get("status") == "FILLED":
                                msg = f"Compra automatica ejecutada: {qty} c de {ticker} a ${res.get('price'):.2f}."
                                c.execute("INSERT INTO bot_logs (bot_id, severity, event_type, message) VALUES ('paper_bot', 'INFO', 'BUY_FILLED', ?)", (msg,))
                            else:
                                msg = f"Compra rechazada: {res.get('reason')}"
                                c.execute("INSERT INTO bot_logs (bot_id, severity, event_type, message) VALUES ('paper_bot', 'WARNING', 'BUY_REJECTED', ?)", (msg,))
                            db_conn.commit()
                        except Exception as buy_e:
                            print(f"[Worker paper_bot] Error de ejecucion de compra: {buy_e}")
                            if db_conn:
                                try:
                                    db_conn.close()
                                except Exception:
                            ...
                            try:
                                db_conn = sqlite3.connect(DB_NAME, timeout=30.0)
                                c = db_conn.cursor()
                                c.execute("INSERT INTO bot_logs (bot_id, severity, event_type, message) VALUES ('paper_bot', 'ERROR', 'BUY_ERROR', ?)", (f"Error de compra: {str(buy_e)}",))
                                db_conn.commit()
                            except Exception as log_err:
                                print(f"[Worker paper_bot] No se pudo escribir log de error de compra: {log_err}")
                        finally:
                            if db_conn:
                                db_conn.close()
            ...
            if desired == 'RUNNING':
                ...
                if time.time() - last_scan_time >= scan_interval:
                    ...
                    try:
                        ...
                        c.execute("UPDATE bot_status SET last_error = NULL WHERE bot_id = 'live_scanner'")
                        conn.commit()
                        conn.close()
