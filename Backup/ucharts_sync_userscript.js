...
(function() {
  ...
  // Activación del control real. No inyectar eventos sintéticos ni coordenadas:
  // UCharts/React puede ignorarlos o asociarlos con un elemento distinto.
  ...
  function init() {
    ...
    panel.innerHTML = `
      <div class="sync-tabs">
        <div class="sync-tab active" data-tab="tab-sheet">Planilla 📊 v5.0.4</div>
  ...
  async function setOrderType(desiredLabels) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const optionCandidates = Array.from(document.querySelectorAll(
        '[role="option"], [role="listbox"] li, [role="listbox"] button, [role="listbox"] div, li, button'
      )).filter(isUsableControl);
  ...
  async function executeOrder(ticker, type, dteProfile, quantity, targetExpiryKey = null, options = {}) {
    ...
    try {
      ...
      if (submitSuccess) {
        ...
        if (!skipAutoTP) {
          console.info(`[Auto-Trader] Programando colocación de venta límite de Take Profit...`);
          setTimeout(() => {
            scheduleAutoTP(ticker);
          }, 4000); // 4 segundos para que se asiente la orden y aparezca en la lista de posiciones
        }
        return { expiryKey: optionSelected.expiryKey, ticker, type, submitted: true };
      } else {
  ...
  // Espera a que el formulario de Negociar/Vender quede libre (por si otra
  // orden sigue en curso, p.ej. la segunda pata del Tester) antes de disparar
  // el cierre automático de Take Profit. Sin esto, el timer de 4s podía
  // dispararse en medio de otra compra y pisarle el formulario.
  async function scheduleAutoTP(ticker) {
    const free = await waitForTradeFormFree();
    if (!free) {
      console.warn('[Auto-Trader] Se descartó la colocación automática de TP: el formulario siguió ocupado por más de 60s.');
      return;
    }
    colocarOrdenLimiteVentaTakeProfit(ticker);
  }
  ...
  async function closePositionByCard(cardElement) {
    await waitForTradeFormFree();
    isTradeFormBusy = true;
    try {
      ...
      if (targetPrice > 0) {
        ...
        const input = priceLabel && priceLabel.parentElement.querySelector('input[type="number"], input[type="text"]');
        if (!input) {
      ...
      // 5. Proceder a revisar y confirmar la orden de venta límite
  ...
  async function monitorPositionsPnL() {
    ...
    for (let descEl of descElements) {
      ...
      if (shouldClose) {
        ...
        const success = await closePositionByCard(card);
  ...
  async function pollPendingTradesFromServer() {
    ...
    try {
      ...
      if (pending && pending.length > 0) {
        ...
        // Reportar el resultado de la compra al bot de Python para notificar a Telegram
        try {
  ...
  async function panicCloseAll() {
    ...
    for (let descEl of descElements) {
      ...
      if (card) {
        const ok = await closePositionByCard(card);
        if (!ok) success = false;
